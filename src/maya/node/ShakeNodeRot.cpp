...
// Private Data
PerlinNoise ShakeNodeRot::__ipNoise;
...
bool ShakeNodeRot::isPassiveOutput(const MPlug& plug) const
{
	...
	Returns:
		bool: Wheter or not the specified plug is passive - true indicates passive.

	*/
	if (plug == outputAttr || plug == outputAttrX || plug == outputAttrY || plug == outputAttrZ)
	{
		return true;
	}
		...
		else
		{
			...
			for (unsigned int i = 0; i < numShakeLayers; ++i)
			{
				...
				if (weight != 0)
				{
					...
					resultX += __ipNoise.calculateNoise(weight, uiTime, seed + 13.0, freq, strX, fractal, rough);
					resultY += __ipNoise.calculateNoise(weight, uiTime, seed + 578.0, freq, strY, fractal, rough);
					resultZ += __ipNoise.calculateNoise(weight, uiTime, seed + 1511.0, freq, strZ, fractal, rough);
				}
			}
...
void ShakeNodeRot::getCacheSetup(const MEvaluationNode& evalNode, MNodeCacheDisablingInfo& disablingInfo, MNodeCacheSetupInfo& cacheSetupInfo, MObjectArray& monitoredAttributes) const
{
